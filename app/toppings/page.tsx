...
export default async function ToppingsPage() {
  ...
  const { data: toppings } = await supabase
    ...
    .select("*")
    .eq("is_active", true)
    .or("slug.ilike.%topping%,name.ilike.%topping%,slug.ilike.%extra%,name.ilike.%extra%")
    .order("name")
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1 container mx-auto px-4 py-12 mt-24 md:mt-32">
        <div className="text-center mb-12">
          <h1 className="text-5xl md:text-6xl font-serif font-bold mb-4">Toppings</h1>
          <p className="text-muted-foreground max-w-2xl mx-auto text-lg">
            Extra toppings and sides to customize your meal just the way you like it.
          </p>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          ...
          {(!toppings || toppings.length === 0) && (
            <div className="col-span-full text-center py-12">
              <p className="text-muted-foreground italic text-lg">New toppings are being added to our menu!</p>
            </div>
          )}
        </div>
      </main>
