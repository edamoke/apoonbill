...
import { Plus, Trash2, Upload, MoveUp, MoveDown, Loader2, Star } from "lucide-react"

export function FeaturedMenuEditor({ initialContent }: { initialContent: any }) {
  const [content, setContent] = useState({
    title: initialContent?.title || "Featured Menu",
    items: initialContent?.items || [
      { title: "Pilau with Chicken", img: "/placeholder.jpg" },
      { title: "Ugali with Fish", img: "/placeholder.jpg" }
    ]
  })
  ...
  const handleSave = async () => {
    setLoading(true)
    try {
      const result = await updateSiteSetting("featured_menu", content)
      if (result.success) {
        toast.success("Featured Menu updated")
      } else {
  ...
  const addItem = () => {
    setContent({
      ...
      items: [
        ...content.items,
        { title: "New Featured Item", img: "/placeholder.jpg" }
      ]
    })
  }
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      <Card className="border-2 border-primary/10 shadow-lg">
        <CardHeader className="bg-primary/5">
          ...
          <CardDescription>Manage the scrolling featured items on the home page</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6 pt-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between border-b pb-2">
              <h4 className="font-bold uppercase text-xs tracking-widest text-muted-foreground">Menu Items</h4>
                  ...
                  <div className="w-32 h-40 rounded-xl overflow-hidden bg-muted flex-shrink-0 relative border shadow-inner">
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                      ...
                      <Button 
                        ...
                        onClick={() => document.getElementById(`featured-img-${index}`)?.click()}
